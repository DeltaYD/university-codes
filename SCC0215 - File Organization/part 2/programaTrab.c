...
int main(void){
    FILE *source, *destiny;
    CABECALHO c;
    char s[50], d[50], quit[128], skip[111];
    int o, n = 0, RRN = 0;
     ...
     *      NEW 7) buscaCombinada(): Processa uma busca
     *          usando o nome dos campos.
     *      NEW 8) insereRegistroBinario(): Faz a insercao
     ...
     *      NEW 5) n: Indica a quantidade de operacoes que vao
     *          ser feitas de busca, insercao, remocao ou atua-
     *          lizacao.
     *      NEW 6) RRN: Recebe uma entrada que se trata do
     *          RRN onde se encontra a informacao buscada.
     *      NEW 7) skip: Usado para substituir um fseek()
    ...
    //  NEW Opcao 3: Busca combinada
    //  NEW Opcao 4: Busca por RRN
    //  NEW Opcao 5: Remocao de registros
    //  NEW Opcao 6: Insercao de registros
    //  NEW Opcao 7: Atualizacao de registros
    scanf("%d", &o);
